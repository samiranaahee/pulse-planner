...
const FocusTracker = () => {
  ...
  const [currentTimer, setCurrentTimer] = useState(25 * 60); // 25 minutes in seconds
  ...
  useEffect(() => {
    if (timerRunning && currentTimer > 0) {
      timerRef.current = setInterval(() => {
        setCurrentTimer((prev) => {
          if (prev <= 1) {
            ...
            // Play notification and switch timer type
            if (timerType === "focus") {
              playNotification(
      ...
      // Clear timer when stopped or finished
      if (timerRef.current) {
  ...
  }, [timerRunning, currentTimer, timerType]);

  // Focus Mode Functions
  const enterFocusMode = () => {
    setFocusModeActive(true);
    setCurrentTimer(25 * 60); // Reset to 25 minutes
    setTimerType("focus");
    setTimerRunning(false); // Start stopped
  };
  ...
  const resetTimer = () => {
    setTimerRunning(false);
    if (timerType === "focus") {
      setCurrentTimer(25 * 60); // 25 minutes
    } else {
      setCurrentTimer(5 * 60); // 5 minutes
    }
  };
  ...
  // If Focus Mode is active, show only the focus mode screen
  if (focusModeActive) {
    return (
      <div
        style={{
      ...
      >
        {/* Focus Mode Header */}
        <h1 style={{ fontSize: "40px", marginBottom: "30px" }}>FOCUS MODE</h1>

        {/* Current Session Type */}
        <h2
          style={{
            fontSize: "24px",
            color: timerType === "focus" ? "#ff6b6b" : "#4ecdc4",
            marginBottom: "20px",
          }}
        >
        ...
        >
          {formatTimerTime(currentTimer)}
        </div>

        {/* Timer Status */}
        <p style={{ fontSize: "20px", marginBottom: "30px" }}>
          {timerRunning ? "TIMER IS RUNNING" : "TIMER IS STOPPED"}
        </p>
        ...
        >
          <p>
            Today's Completed Pomodoros: <strong>{pomodoroCount}</strong>
          </p>
          <p>
            Total Focus Time: <strong>{formatTime(focusMinutes)}</strong>
          </p>
        </div>
  ...
  // Regular view when Focus Mode is not active
  return (
    <div style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
      ...
      >
        <h2 style={{ color: "red", fontSize: "22px" }}>Focus Mode</h2>
        <p>Hide all distractions and focus on your timer only!</p>
        ...
        <p style={{ fontSize: "12px", color: "#666", marginTop: "10px" }}>
          This will hide everything except the timer. Perfect for deep focus!
        </p>
      </div>
